...
namespace Atlassian.Jira.Remote
{
    internal class ProjectComponentService : IProjectComponentService
    {
        ...
        {
            var resource =
                $"/rest/api/2/component/{componentId}?{(string.IsNullOrEmpty(moveIssuesTo) ? null : "moveIssuesTo=" + Uri.EscapeDataString(moveIssuesTo))}";
        ...
        public async Task<IEnumerable<ProjectComponent>> GetComponentsAsync(string projectKey, CancellationToken token = default)
        {
            ...
            if (!cache.Components.Values.Any(c => string.Equals(c.ProjectKey, projectKey)))
            {
                ...
                var remoteComponents = await _jira.RestClient.ExecuteRequestAsync<RemoteComponent[]>(Method.Get, resource, token: token).ConfigureAwait(false);
